...
for (i in 1:nrow(raw_scores)){
  raw_scores$Homework[i] <- score_homework(drop_lowest(c(one[i],two[i],three[i],four[i],five[i],six[i],seven[i],eight[i],nine[i])))
}
...
for (i in 1:nrow(raw_scores)){
  raw_scores$Quiz[i] <- score_quiz(drop_lowest(c(quiz_1[i],quiz_2[i],quiz_3[i],quiz_4[i])))
}
